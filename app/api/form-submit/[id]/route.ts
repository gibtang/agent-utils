...
export async function POST(
  ...
  try {
    ...
    if (!form) {
      return NextResponse.json({ error: 'Form not found', url: process.env.NEXT_PUBLIC_APP_URL }, { status: 404 });
    }

    const now = new Date();
    if (form.status !== 'active' || (form.expiresAt && form.expiresAt < now)) {
      return NextResponse.json({ error: 'Form no longer available', url: process.env.NEXT_PUBLIC_APP_URL }, { status: 410 });
    }
