...
assert resp.status_code == 200`,
  ...
  },
  {
    ...
    docsUrl: '/docs/audit-log',
    metaTitle: 'AI Agent Audit Log API | Immutable Decision Trail | AgentUtils',
    metaDescription: 'Record AI agent decisions in an immutable, server-timestamped audit log. Keep structured provenance for debugging, compliance, and incident review.',
    ...
    useCases: [
      { title: 'Decision provenance', description: 'Record each agent decision and rationale so reviewers can explain why an action was taken later' },
      { title: 'Access logging', description: 'Log every time an agent reads or writes sensitive data for compliance evidence' },
      { title: 'Incident reconstruction', description: 'Replay the sequence of agent actions around an outage or bad outcome by time range' },
...
const { data } = await res.json();`,
  ...
  },
  {
    ...
    subtitle: 'Your agent can ask for help before it guesses wrong.',
    whatItDoes: 'An agent reports its uncertainty with structured concerns and context. A tenant reviewer resolves it with continue, pivot, or abort guidance, and the agent polls to resume.',
    whyAgentsNeed: ['Cloud agents cannot surface uncertainty to a nearby human', 'Long-running work needs a stateful review round-trip', 'Human judgment can prevent expensive loops and unsafe guesses'],
...
console.log(data.review_url);`,
    ...
    keywords: ['agent confession api', 'cloud agent steering', 'human guidance for agents', 'agent uncertainty reporting'],
    relatedTools: ['checkpoint', 'dlq', 'audit-log'],
  },
  {
    slug: 'scheduler',
    name: 'Scheduler',
    ...
    docsUrl: '/docs/v2',
    metaTitle: 'AI Agent Scheduler API | Delayed Callbacks & Retries | AgentUtils',
    metaDescription: 'Schedule one-off callbacks for AI agents with delayed execution, fixed retries, and optional dead-letter fallback. One HTTP API for deferred workflows.',
    ...
    useCases: [
      { title: 'Delayed follow-up', description: 'Schedule a reminder or status check without keeping a worker process alive' },
      ...
      { title: 'Workflow checkpoints', description: 'Wake a paused agent step when the next action becomes due' },
    ],
    codeExample: {
...
const { data } = await res.json();`,
  ...
  },
  {
    ...
    metaTitle: 'Image Upload API for AI Agents | Hosted URLs | AgentUtils',
    metaDescription: 'Upload screenshots, generated images, or user media and receive a durable hosted URL in one API call. No SDK or storage setup.',
    ...
    useCases: [
      { title: 'Generated image handoff', description: 'Upload model outputs and pass a hosted URL to the next agent step' },
      { title: 'Screenshot sharing', description: 'Store screenshots and send a durable link in a report or message' },
      { title: 'User media handoff', description: 'Persist user-provided images for later processing without local storage' },
      { title: 'Pipeline outputs', description: 'Pass hosted image URLs between services and human reviewers' },
    ],
    codeExample: {
