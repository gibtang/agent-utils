...
spec = {
    ...
    'tags': [
        ...
        {'name': 'Connections', 'description': 'One-time pairing and connection status'},
        {'name': 'Secure Handoffs', 'description': 'Encrypted credential handoffs between agents and owners'},
    ...
    'components': {
        'securitySchemes': {
            ...
            'connectionCredential': {'type': 'http', 'scheme': 'bearer', 'bearerFormat': 'au_conn_…', 'description': 'Connection credential returned once when a pairing code is redeemed'},
        ...
        'schemas': {
            ...
            'ConfessionCreate': {'type': 'object', 'required': ['title', 'callback_url'], 'properties': {
                'title': {'type': 'string', 'maxLength': 256}, 'summary': {'type': 'string', 'maxLength': 4096},
                'context': {}, 'urgency': {'type': 'string', 'enum': ['normal', 'high', 'blocking'], 'default': 'normal'},
                'reviewer_email': {'type': 'string', 'format': 'email'}, 'expires_in_seconds': {'type': 'integer', 'minimum': 300, 'maximum': 604800, 'default': 86400},
                'callback_url': {'type': 'string', 'format': 'uri'}, 'callback_payload': {}}},
            'ConfessionResolve': {'type': 'object', 'required': ['decision'], 'properties': {
                'decision': {'type': 'string', 'enum': ['approved', 'rejected']}, 'by': {'type': 'string', 'maxLength': 128}, 'note': {'type': 'string', 'maxLength': 1024}}},
    ...
    'paths': {
        '/connections/pair': {
            'post': {'tags': ['Connections'], 'summary': 'Pair a runtime with an agent using a one-time code',
                'description': 'Public endpoint. Exchanges a single-use pairing code for a connection credential. Codes expire after 10 minutes.',
                'security': [],
                'requestBody': {'required': True, 'content': {'application/json': {'schema': {'type': 'object', 'required': ['code'],
                    'properties': {'code': {'type': 'string'}, 'runtimeVersion': {'type': 'string'}}}}}},
                'responses': {'201': {'description': 'Paired — returns the connection credential once'},
                    '400': {'description': 'Validation failed'}, '404': {'description': 'Invalid or expired pairing code'},
                    '429': {'description': 'Rate limit exceeded'}}}},
        '/connections/status': {
            'get': {'tags': ['Connections'], 'summary': 'Return status for the calling connection',
                'description': 'Requires a Bearer `au_conn_…` connection credential.',
                'security': [{'connectionCredential': []}],
                'responses': {'200': {'description': 'Connection, agent, plan and capabilities'},
                    '401': {'description': 'Authentication required'}, '403': {'description': 'Connection revoked or inactive'}}}},
        '/handoffs': {
            'post': {'tags': ['Secure Handoffs'], 'summary': 'Create an encrypted credential handoff',
                'description': 'Creates a handoff owned by the calling connection and returns one-time view and submit tokens.',
                'security': [{'connectionCredential': []}],
                'requestBody': {'required': True, 'content': {'application/json': {'schema': {'type': 'object', 'required': ['fieldSchema'],
                    'properties': {'fieldSchema': {'type': 'array', 'maxItems': 12, 'items': {'type': 'object'}},
                        'title': {'type': 'string'}, 'linkTtlHours': {'type': 'number'}, 'callbackUrl': {'type': 'string', 'format': 'uri'}}}}}},
                'responses': {'201': {'description': 'Created — one-time handoff tokens'}, '400': {'description': 'Validation failed'},
                    '401': {'description': 'Authentication required'}}},
            'get': {'tags': ['Secure Handoffs'], 'summary': 'List handoffs for the calling account',
                'security': [{'connectionCredential': []}],
                'responses': {'200': {'description': 'Metadata-only handoff list'}, '401': {'description': 'Authentication required'}}}},
        '/handoffs/status': {
            'post': {'tags': ['Secure Handoffs'], 'summary': 'Return metadata-only handoff status',
                'security': [{'connectionCredential': []}],
                'requestBody': {'required': True, 'content': {'application/json': {'schema': {'type': 'object', 'required': ['handoffId'],
                    'properties': {'handoffId': {'type': 'string'}}}}}},
                'responses': {'200': {'description': 'Handoff status without secret values'}, '404': {'description': 'Not found'},
                    '401': {'description': 'Authentication required'}}}},
        '/handoffs/close': {
            'post': {'tags': ['Secure Handoffs'], 'summary': 'Close a handoff created by the calling connection',
                'security': [{'connectionCredential': []}],
                'requestBody': {'required': True, 'content': {'application/json': {'schema': {'type': 'object', 'required': ['handoffId', 'reason'],
                    'properties': {'handoffId': {'type': 'string'}, 'reason': {'type': 'string', 'enum': ['done']}}}}}},
                'responses': {'200': {'description': 'Closed and crypto-erased'}, '404': {'description': 'Not found'},
                    '401': {'description': 'Authentication required'}}}},
        '/handoffs/decrypt': {
            'post': {'tags': ['Secure Handoffs'], 'summary': 'Decrypt values submitted to a handoff',
                'security': [{'connectionCredential': []}],
                'requestBody': {'required': True, 'content': {'application/json': {'schema': {'type': 'object', 'required': ['handoffId'],
                    'properties': {'handoffId': {'type': 'string'}}}}}},
                'responses': {'200': {'description': 'Plaintext values for the creating connection'}, '404': {'description': 'Not found or expired'},
                    '401': {'description': 'Authentication required'}}}},
        '/tenants': {
        ...
        '/confessions': {
            'post': {'tags': ['Confessions'], 'summary': 'Create an uncertainty report',
                'description': 'Persists the Confession, returns a one-time review URL, and queues a default reviewer email asynchronously. The agent can poll or receive a signed resolution callback.',
                'security': [{'agentKey': []}],
                'requestBody': {'required': True, 'content': {'application/json': {'schema': {'$ref': '#/components/schemas/ConfessionCreate'}}}},
                'responses': {'201': {'description': 'Created — includes review_url'}, '402': {'description': 'QUOTA_EXCEEDED (confessions_pending)'}}},
            'get': {'tags': ['Confessions'], 'summary': 'List tenant confessions', 'security': [{'agentKey': []}],
                'parameters': [{'name': 'status', 'in': 'query', 'schema': {'type': 'string', 'enum': ['pending', 'resolved', 'cancelled', 'expired']}}, {'name': 'agent_id', 'in': 'query', 'schema': {'type': 'string'}}, {'name': 'cursor', 'in': 'query', 'schema': {'type': 'string'}}], 'responses': {'200': {'description': 'List'}}}},
        '/confessions/{id}': {
            ...
            'delete': {'tags': ['Confessions'], 'summary': 'Cancel a pending confession (creating agent only)', 'security': [{'agentKey': []}], 'parameters': [P('confessionId')], 'responses': {'204': {'description': 'Cancelled'}, '403': {'description': 'Not the creating agent'}, '409': {'description': 'Already resolved'}}}},
        '/confessions/{id}/resolve': {'post': {'tags': ['Confessions'], 'summary': 'Resolve a confession',
            'description': 'Admin key, approval-proxy key, or a one-time reviewer token. Resolution fires a signed callback and cancels pending notification work.',
            'security': [{'adminKey': []}, {'approvalKey': []}], 'parameters': [P('confessionId')],
            'requestBody': {'required': True, 'content': {'application/json': {'schema': {'$ref': '#/components/schemas/ConfessionResolve'}}}}, 'responses': {'200': {'description': 'Resolved'}, '409': {'description': 'Already resolved'}, '404': {'description': 'Cross-tenant / not found'}}}},
